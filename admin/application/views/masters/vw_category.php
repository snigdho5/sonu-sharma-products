		...
		<div class="container-fluid">
			...
			<div class="row">
				<div class="col-12">
					<div class="card">
						<div class="card-body">
							<h5 class="card-title">Category <button type="button" class="btn badge badge-pill badge-success" onclick="location.href='<?php echo base_url().'addcategory'; ?>'">Add Category</button></h5>
									...
									if(!empty($cat_data)){
										...
										foreach($cat_data as $key => $val){
											...
											<tr>
												...
												<?php if (!empty($this->session->userdata('userid')) && $this->session->userdata('usr_logged_in')==1 && $this->session->userdata('usergroup')==1) {  ?>
													<td>
														<button type="button" onclick="location.href='<?php echo base_url().'editcategory/'.$val['category_id']; ?>'"><i class="icofont-pencil-alt-2"></i></button>
														<button type="button" class="del_cat" data-catid="<?php echo $val['category_id']; ?>" data-catname="<?php echo $val['category_name']; ?>"><i class="fas fa-trash-alt"></i></button>
													</td>
												<?php } ?>
...
<script>
 ...
 $(document).ready(function(){
	...
	$(document).on('click', '.del_cat', function(){
		...
		conf = confirm('Are you sure to delete '+catname+'?');
		if(conf){
			$.ajax({
